...
reconstruction to tools/collect/collect_va_table.py).

Tags produced by eval_dcbg: DCBG_mask{B}_g{gamma}_adj[_fo]{res_suffix}_raw
with res_suffix _v4 (seen) / _v4uns (unseen).

  python tools/collect/collect_dcbg_table.py -gamma 1.0 -out dcbg_table_v4_f005.json
"""
...
if __name__ == "__main__":
    ...
    ap.add_argument("-sfx_seen", type=str, default="_v4", help="res_suffix of the seen runs")
    ap.add_argument("-sfx_uns", type=str, default="_v4uns", help="res_suffix of the unseen runs")
    ...
    for variant, fo in [("exact", ""), ("fo", "_fo")]:
        for regime, sfx in [("seen", args.sfx_seen), ("unseen", args.sfx_uns)]:
