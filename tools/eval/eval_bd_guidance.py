...
tools/train/train_bd_disc.py's split, so the discriminator never saw them), and scores
...
shortest paths, plus the removed-edge usage rate (the direct guidance target).
  ...
  baseline: python tools/eval/eval_bd_guidance.py -blk 4 -family 0.05 -disc none
  guided  : python tools/eval/eval_bd_guidance.py -blk 4 -family 0.05 \
