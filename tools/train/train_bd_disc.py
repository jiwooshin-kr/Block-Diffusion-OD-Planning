...
Train the partial-path discriminator for BD importance-weight guidance.
  ...
  python tools/train/train_bd_disc.py -family 0.05 -frac 1 -exp e0 -neg data
...
def load_sp(porto, family, tag, dver="v4"):
    return pickle.load(open(join(porto, f"porto_shrink_SP_{dver}-{family}_{tag}.pkl"), "rb"))


def load_A(porto, family, tag, dver="v4"):
    A = pickle.load(open(join(porto, f"porto_shrink_A_{dver}-{family}_{tag}.ts"), "rb"))
    return A.bool().float()
...
if __name__ == "__main__":
    ...
    ap.add_argument("-n2v", type=str, default="./sets_data/porto_node2vec.pkl",
                    help="node2vec 캐시 (v6는 노드 수가 달라 별도 파일)")
    ...
    normal_paths = [list(map(int, p)) for p in load_sp(args.porto, fam, "normal", args.dver) if len(p) >= 2]
