...
(uncond_pool_blk{B}.pth) differs. Mirrors tools/train/train_bd_disc.py exactly (same
make_partial canvas, per-batch 50/50, adjacency conditioning, BCE, bounds).

  python tools/train/train_e99_model_multi.py -jobs 4:sets_disc/uncond_pool_blk4.pth,...
...
def load_sp(porto, fam, tag, dver="v4"):
    return pickle.load(open(join(porto, f"porto_shrink_SP_{dver}-{fam}_{tag}.pkl"), "rb"))


def load_A(porto, fam, tag, dver="v4"):
    return pickle.load(open(join(porto, f"porto_shrink_A_{dver}-{fam}_{tag}.ts"), "rb")).bool().float()
...
if __name__ == "__main__":
    ...
    ap.add_argument("-n_exc", type=int, default=99, help="positives로 쓸 예외 시나리오 수 (스모크용 축소)")
    ...
    sp0 = load_sp(args.porto, fam, "except_0", args.dver)
    ...
    A0 = load_A(args.porto, fam, "except_0", args.dver)
