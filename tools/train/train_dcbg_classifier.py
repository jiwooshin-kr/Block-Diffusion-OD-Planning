...
generation time. Same 1% data budget / split seeds as tools/train/train_bd_disc.py.

  python tools/train/train_dcbg_classifier.py -kernel mask  -blk 4
  python tools/train/train_dcbg_classifier.py -kernel mask  -blk 64
  python tools/train/train_dcbg_classifier.py -kernel graph -blk 64 \
...
if __name__ == "__main__":
    ...
    ap.add_argument("-exp", type=str, default="e0", choices=["e0", "e99"], help="e99 = train on except 1..99 (unseen-scenario exp. 2)")
    ap.add_argument("-n_exc", type=int, default=99, help="debug limiter for e99")
    ap.add_argument("-neg", type=str, default="data", choices=["data", "model"],
                    help="model = negatives are the block's unconditional generations (same pools as our modelD discs), corrupted like the positives")
    ...
    ap.add_argument("-n2v", type=str, default="./sets_data/porto_node2vec.pkl",
                    help="node2vec 캐시. v6 는 ./sets_data_v6/porto_node2vec.pkl (1387 노드)")
    ap.add_argument("-out", type=str, default="./sets_disc", help="분류기 저장 디렉터리")
    args = ap.parse_args()
    ...
    if args.neg == "model":
        pool = args.pool or f"./sets_disc/uncond_pool_blk{args.blk}.pth"
        neg_paths = [list(map(int, p)) for p in torch.load(pool)["paths"] if len(p) >= 2]
        print(f"[dcbg-clf] model-negative pool: {len(neg_paths)} ({pool})", flush=True)
    else:
        sp_norm = pickle.load(open(join(args.porto, f"porto_shrink_SP_{dver}-{fam}_normal.pkl"), "rb"))
        neg_paths = [list(map(int, p)) for p in sp_norm if len(p) >= 2]
    exceptions = [0] if args.exp == "e0" else list(range(1, args.n_exc + 1))
    exc_paths, exc_A, exc_dr = {}, {}, {}
    A_nrm_ = pickle.load(open(join(args.porto, f"porto_shrink_A_{dver}-{fam}_normal.ts"), "rb")).bool().float()
    for e in exceptions:
        sp_e = pickle.load(open(join(args.porto, f"porto_shrink_SP_{dver}-{fam}_except_{e}.pkl"), "rb"))
        perm_e = np.random.RandomState(SPLIT_SEED + e).permutation(len(sp_e))
        pool = perm_e[EVAL_RESERVE:] if e == 0 else perm_e
        n_train = max(1, int(np.ceil(args.frac / 100.0 * len(sp_e))))
        exc_paths[e] = [list(map(int, sp_e[i])) for i in pool[:n_train] if len(sp_e[i]) >= 2]
        A_e = pickle.load(open(join(args.porto, f"porto_shrink_A_{dver}-{fam}_except_{e}.ts"), "rb")).bool().float()
        exc_A[e] = A_e
        exc_dr[e] = (A_e.sum(1) / A_nrm_.sum(1).clamp(min=1))
        if e == 0:
            sp_exc = sp_e
    if args.exp == "e99":
    ...
    print(f"[dcbg-clf] exp={args.exp} exceptions={len(exceptions)} "
          f"paths/exc={np.mean([len(v) for v in exc_paths.values()]):.0f}", flush=True)
    ...
    # 백본과 토큰 id 공간을 공유해야 한다(dcbg_plugin 이 model 의 canvas 를
    # 그대로 clf 에 넣으므로 END/PAD/MASK 가 어긋나면 조용히 틀린다).
    n_vertex = A_nrm_.shape[0]
    A_exc = pickle.load(open(join(args.porto, f"porto_shrink_A_{dver}-{fam}_except_0.ts"), "rb")).bool().float()
    A_nrm = pickle.load(open(join(args.porto, f"porto_shrink_A_{dver}-{fam}_normal.ts"), "rb")).bool().float()
    adj_dev = A_exc.to(device)
    dr_dev = (A_exc.sum(1) / A_nrm.sum(1).clamp(min=1)).to(device)
    if args.adj:
        clf = DCBGClassifierAdj(n_vertex, device, pretrain_path=args.n2v)
    else:
        clf = DCBGClassifier(n_vertex, device, pretrain_path=args.n2v)
    ...
    for step in range(1, args.steps + 1):
        ...
        pos_paths = exc_paths[e]
        if args.adj:
            adj_dev = exc_A[e].to(device)
            dr_dev = exc_dr[e].to(device)
        ...
        if step % 500 == 0 or step == args.steps:
            ...
            with torch.no_grad():
                ...
                vz = (clf(vt.to(device), vts.to(device), A_exc.to(device),
                          (A_exc.sum(1) / A_nrm.sum(1).clamp(min=1)).to(device))
                      if args.adj else clf(vt.to(device), vts.to(device)))
    ...
    out = join(args.out, f"DCBGclf_{args.kernel}_blk{args.blk}_f{fam}_p{int(args.frac)}{sfx}.pth")
    torch.save(clf, out)
    print(f"saved {out}")
