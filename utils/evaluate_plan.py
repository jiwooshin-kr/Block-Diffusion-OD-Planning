...
class Evaluator:
    ...
    def eval(self, n_samples, suffix):
        # planned_paths, orig_paths = self.eval_exsits(6) # org : eval_exsits(500)
        planned_paths, orig_paths = self.eval_exsits(n_samples)
        ...
        # org : 원래 아래 activate 해야함.
        # 아래 hit rate 계산할 때 division by zero 떠서 inactivate해놓음.
    ...
    def eval_exsits(self, n_samples):
        choices = np.random.choice(len(self.dataset), [len(self.dataset) - 1], False).tolist() #org : 2번째 arg가 [n_samples]
        # evaluate batchly
        set_batch_size = 15
        n_batch = (n_samples + set_batch_size - 1) // set_batch_size
        planned_paths = []
        n_samples = len(self.dataset) - 1 # org : 이 코드 없었음. indexerror 나서 n_sample 크기 바꾼것.
