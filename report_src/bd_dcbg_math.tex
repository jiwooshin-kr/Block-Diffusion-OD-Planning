...
\begin{remark}[블록 적합성: mask 는 맞고 graph 는 non-block 원본이 남음]\label{rem:blockfit}
block diffusion 에 맞는 노이즈 상태는 \textbf{이전 블록 clean 고정 + 현재 블록만 노이즈 + 미래 블록 부재}이다.
\begin{itemize}[nosep,leftmargin=1.4em]
  \item \textbf{mask}: \code{corrupt\_mask}(\code{:107})가 $j$개 확정 블록을 clean 유지,
  현재 블록만 rate $t$ 마스킹, 미래 truncate 하여 이 구조를 정확히 구현한다. 추론
  (\code{plan\_dcbg\_mask})도 full canvas(prefix+현재 블록)를 넣어 학습과 일치한다.
  \item \textbf{graph}: \code{corrupt\_graph}(\code{:145})는 \code{row[2:]=noised[2:]} 로
  \emph{경로 전체}를 단일 $t$ 로 노이즈한다(dst,ori 만 clean) — 이는 블록 구조가 아니라
  \emph{non-block MDLM/uniform 원본} corruption 이다. 게다가 추론
  (\code{plan\_dcbg\_graph:320})은 \code{seq[:,-block:]} 로 \emph{현재 블록만} 넣어
  prefix 를 무시하고 학습/추론 입력이 불일치한다. 즉 graph D-CBG 는 블록 확산에 미적응 상태이며,
  이것이 §5 의 graph 무력성에 (Taylor 근사 무력과 함께) 겹친다.
\end{itemize}
\end{remark}

\begin{remark}[Block-size 독립성: IW 는 가능, D-CBG 는 불가]\label{rem:blockdep}
IW disc 는 clean $x_0$ 공간의 비율을 다루고 입력이 \emph{clean 부분 경로}라
(\code{bd\_disc.py:119} forward 에 \code{block} 인자 없음) \textbf{block size 에 무관}하다 —
실제로 자매 문서 실험 §4.1b 에서 단일 disc 하나로 blk 4/8/16/32/64 를 모두 평가했다.
반면 D-CBG 는 $p_\phi(y\mid x_t,t)$ 를 다루는데 \textbf{$x_t$ 의 구조(어디가 확정 clean 블록/현재 노이즈 블록/미래 부재인지)가 block size 로 정의}되므로,
\code{corrupt\_*} 가 \code{block} 을, \code{train\_dcbg\_classifier.py} 가 \code{-blk} 를 요구하며
\textbf{블록마다 별도 classifier}(\code{DCBGclf\_*\_blk\{B\}})가 필요하다. 이는 구현 편의가 아니라
정식화의 구조적 차이이며, 블록 확산에서 $x_0$-space(IW) 정식화가 갖는 실질적 이점이다.
\end{remark}
