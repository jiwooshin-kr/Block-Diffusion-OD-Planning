...
\end{lemma}
\noindent \textbf{구체적 마스크(mask 커널, reveal당).} 블록 위치 $\ell$ 의 이미 공개된
좌/우 이웃을 $u_L,u_R$ 라 하면, 후보 정점 $k$ 의 legality 마스크는
\begin{equation}
m_\ell(k)=\Big[\,u_L\text{ 공개}\Rightarrow A_\mathrm{scn}(u_L,k)\,\Big]\cdot
          \Big[\,u_R\text{ 공개}\Rightarrow A_\mathrm{scn}(k,u_R)\,\Big],\qquad
p_{\text{cand}}\leftarrow \frac{p_{\text{cand}}\odot m_\ell}{\sum_k (p_{\text{cand}}\odot m_\ell)}
\end{equation}
즉 후보 $k$ 는 \emph{공개된 양쪽 이웃과 시나리오 edge 로 연결될 때만} 살아남고, 그 외에는
$p_{\text{cand}}$ 에서 확률 0 이 된다(제거·미존재 edge 사용을 원천 차단). 공개 안 된 이웃 쪽은
제약 없음(마스크 1). 이 곱형 마스크가 Lemma~\ref{lem:adj} 의 ``$A_\mathrm{scn}$-합법 전이''
제한을 구현한다.

\noindent 구현: \code{plan\_guided(\dots, adj\_prop=True)}; mask 커널 reveal당 마스킹
\code{bd\_models.py:929--956}; graph 최종 디코드의 시나리오 인접성 사용
\code{bd\_models.py:1113--1121}.
...
adj-masked proposal (Lemma~\ref{lem:adj}) & \code{bd\_models.py:929--956 (mask), 1113--1121 (graph)} \\
\bottomrule
\end{tabular}
\end{center}
