...
( for B in 1 2 4 8; do
    CUDA_VISIBLE_DEVICES=0 python -u train_bd_disc.py -family 0.1 -frac 1 -exp e0 -neg model \
      -pool $D/uncond_pool_blk$B.pth > $L/f01_disc_e0_blk$B.log 2>&1 || echo "e0 blk$B FAIL"
  done ) &
( for B in 16 32 64; do
    CUDA_VISIBLE_DEVICES=1 python -u train_bd_disc.py -family 0.1 -frac 1 -exp e0 -neg model \
      -pool $D/uncond_pool_blk$B.pth > $L/f01_disc_e0_blk$B.log 2>&1 || echo "e0 blk$B FAIL"
  done ) &
CUDA_VISIBLE_DEVICES=2 python -u train_e99_model_multi.py -family 0.1 \
...
JOBS=()
for B in 1 2 4 8 16 32 64; do
  JOBS+=("python -u three_way_postproc.py -family 0.1 -ckpt $(CK $B) -disc $D/BDdisc_f0.1_p1_e0_model_blk$B.pth -tag f01m${B}seen")
  JOBS+=("python -u three_way_postproc.py -family 0.1 -ckpt $(CK $B) -disc $D/BDdisc_f0.1_p1_e99_model_blk$B.pth -tag f01m${B}uns")
done
for g in 0 1 2 3; do
  ( for i in "${!JOBS[@]}"; do
      if (( i % 4 == g )); then
