...
run3(){ # $1=gpu $2=blk $3=disc $4=order $5=tag
  CUDA_VISIBLE_DEVICES=$1 python -u tools/eval/three_way_postproc.py -ckpt $(CKm $2) \
...
( for B in 1 2; do for O in first_hit l2r; do
    CUDA_VISIBLE_DEVICES=3 python -u tools/eval/base_only.py -ckpt $(CKm $B) -order $O -tag v3m${B}base_${O} > $R/v3m${B}base_${O}.log 2>&1
  done; done ) &
wait
