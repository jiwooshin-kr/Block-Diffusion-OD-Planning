...
#   (1) uncond pool from the SAME v2 ckpt used at eval (p_theta match; matches scripts/run_v3_all.sh)
...
chain() {   # $1=block  $2=gpu
  ...
  CUDA_VISIBLE_DEVICES=$G python -u tools/train/train_bd_disc.py -family 0.05 -frac 1 -exp e0 -neg model \
    -pool $D/uncond_pool_blk$B.pth > $L/blk12md_disc_e0_$B.log 2>&1 || { echo "[blk$B] e0 disc FAIL"; return 1; }

  echo "[blk$B] (2b) e99 (unseen) disc ..."
  CUDA_VISIBLE_DEVICES=$G python -u tools/train/train_bd_disc.py -family 0.05 -frac 1 -exp e99 -neg model \
  ...
  CUDA_VISIBLE_DEVICES=$G python -u tools/eval/three_way_postproc.py -ckpt $(CK $B) \
