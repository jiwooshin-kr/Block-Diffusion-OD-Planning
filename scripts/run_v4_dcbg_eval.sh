...
#
#   BLOCKS="4 16 64" FAMS="0.05" REGIMES="e0"      bash scripts/run_v4_dcbg_eval.sh   # ~40 min
#   BLOCKS="1 2 4 8 16 32 64" FAMS="0.05 0.1" REGIMES="e0 e99" bash scripts/run_v4_dcbg_eval.sh  # full
#
...
# side (tools/eval/eval_bd_guidance.py -w_gamma default 1.0) -- so the two methods are
...
for V in $VARIANTS; do
  ...
  for F in $FAMS; do for R in $REGIMES; do for B in $BLOCKS; do
    ...
    JOBS+=("python -u tools/eval/eval_dcbg.py -kernel mask -blk $B -family $F -gamma $GAMMA -adj 1 -approx $AP -ckpt $(CK $B) -clf $CLF -res_suffix _v4${F01}${UNS}")
  done; done; done
done
