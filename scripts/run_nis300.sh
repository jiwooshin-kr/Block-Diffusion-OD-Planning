...
for B in 1 2 4 8 16 32 64; do
  JOBS+=("python -u tools/eval/three_way_postproc.py -family 0.05 -n_is 300 -ckpt $(CK $B) -disc $D/BDdisc_f0.05_p1_e0_model_blk${B}_v4.pth  -tag n300m${B}seen")
  JOBS+=("python -u tools/eval/three_way_postproc.py -family 0.05 -n_is 300 -ckpt $(CK $B) -disc $D/BDdisc_f0.05_p1_e99_model_blk${B}_v4.pth -tag n300m${B}uns")
done
for g in 0 1 2 3; do
  ( for i in "${!JOBS[@]}"; do
      if (( i % 4 == g )); then
...
python -u tools/collect/collect_va_table.py -family 0.05 -tag_seen 'n300m{B}seen' -tag_uns 'n300m{B}uns' \
