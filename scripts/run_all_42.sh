...
CUDA_VISIBLE_DEVICES=0 python -u tools/train/train_e99_model_multi.py \
...
PT=$!
CUDA_VISIBLE_DEVICES=1 python -u tools/gen/gen_bd_uncond_pool.py -ckpt $(CKm 16) \
  -out $D/uncond_pool_blk16_200k.pth -n 200000 -batch 800 > $R/gen200k.log 2>&1 &
PG=$!
( for B in 1 2; do CUDA_VISIBLE_DEVICES=2 python -u tools/eval/base_only.py -ckpt $(CKm $B) -tag v3m${B} > $R/base_blk${B}.log 2>&1; done; echo BASE12_DONE ) &
PB=$!
...
for B in 4 8 16 32 64; do
  g=${G[$((i%3))]}; i=$((i+1))
  CUDA_VISIBLE_DEVICES=$g python -u tools/eval/three_way_postproc.py -ckpt $(CKm $B) \
...
CUDA_VISIBLE_DEVICES=1 python -u tools/train/train_e99_model_multi.py \
...
CUDA_VISIBLE_DEVICES=0 python -u tools/eval/three_way_postproc.py -ckpt $(CKm 16) \
