...
for F in 0.05 0.1; do for B in 1 2 4 8 16 32 64; do
  JOBS+=("python -u train_dcbg_classifier.py -kernel mask -blk $B -family $F -adj 1 -neg model -exp e99 -pool $D/uncond_pool_v4_blk$B.pth -outsfx _v4")
done; done
for F in 0.05 0.1; do for B in 1 2 4 8 16 32 64; do
  JOBS+=("python -u train_dcbg_classifier.py -kernel mask -blk $B -family $F -adj 1 -neg model -exp e0 -pool $D/uncond_pool_v4_blk$B.pth -outsfx _v4")
done; done
