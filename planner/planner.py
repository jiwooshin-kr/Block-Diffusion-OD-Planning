...
class Planner(nn.Module):
    ...
    def __init__(self, G: nx.Graph, A: torch.Tensor, restorer: Restorer, destroyer: Destroyer, device: torch.device, x_emb_dim: int, pretrain_path=None):
        ...
        # [Edit - Jiwoo] build_graph_structure 함수로 분리 - why? A가 A'으로 바뀌었을 때 mask information 전체 업데이트 해줘야 해서
        self.build_graph_structure(G, A)
    ...
    # [Added - Jiwoo] init 일부분 새로운 함수로 분리 -> why? A가 A'으로 바뀌었을 때 mask information 전체 업데이트 해줘야 해서
    def build_graph_structure(self, G, A):
    ...
    def plan(self, origs, dests, eval_nll=False, use_refine=True, planning_mode="hybrid"):
        with torch.no_grad():
            window = 1
            ...
            for i in range(1, max_len):
                ...
                if window_idx == window:
                    ...
                    # lengths = torch.Tensor([i - 1 + window]).long().repeat(batch_size).to(self.device)          # org
                    lengths = torch.Tensor([i + window]).long().repeat(batch_size).to(self.device)              # new
                    
                    _, x0_pred_probs = self.restorer.sample_with_len(lengths, xt=real_fake, ret_distr=True)                     # org           
                    # _, x0_pred_probs = self.restorer.sample_with_len(lengths, xt=real_fake, ret_distr=True, prefix=prefix)    # new
                ...
                #  JW 
                if planning_mode == "hybrid":
                    syntheised_probs = diff_probs * gpt_probs
                elif planning_mode == "diffusion":
                    syntheised_probs = diff_probs
                elif planning_mode == "AR":
    ...
    def plan_with_disc(self, origs, dests, disc, eval_nll=False, use_refine=True, planning_mode="hybrid"):
        with torch.no_grad():
            window = 1
            # origs: list of vertices | tensor of vertices
            # dests list of vertices  | tensor of vertices
            ...
            for i in range(1, max_len):
                ...
                if window_idx == window:
                    ...
                    _, x0_pred_probs = self.restorer.sample_with_len_disc(lengths, xt=real_fake, disc=disc, ret_distr=True)                     # org
                    # _, x0_pred_probs = self.restorer.sample_with_len_disc(lengths, xt=real_fake, disc=disc, ret_distr=True, prefix=prefix)      # new
